...
function renderGeography(i, p = null) {
    ...
    const w = world, b = w.basins[w.lakeId[i]], target = w.basins[w.basinTarget[i]], place = typeof PoliticalLand !== 'undefined' ? PoliticalLand.status(w, sim, i) : null, country = place ? place.realm : p ? sim.realms[p.owner] : null, feature = w.features.find(f => f.i === i), plate = w.plates[w.plate[i]], isFjord = w.fjord[i] > 0;
    const enclosedWater = place?.water && place.kind !== 'water' && w.height[i] <= 0;
    const title = feature?.name || b?.name || (enclosedWater ? 'Inland sea' : isFjord ? 'Glacial fjord' : BIOME[w.biome[i]][0]);
