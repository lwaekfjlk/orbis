...
window.CityUI = (() => {
    ...
    function settingPanel(p){
        ...
        return `<section class="town-setting-card"><div class="overline">GEOGRAPHY LOCKED · WORLD CELL ${p.i}</div><h3>${esc(e.label)}</h3><p>Town site: ${BIOME[e.biome][0]} · ${e.temperature.toFixed(1)} °C.<br>Surrounding relief: ${Math.round(e.minElevation).toLocaleString()}–${Math.round(e.maxElevation).toLocaleString()} model m.${e.glacialFoothills?'<br>Glacier-bearing slopes are inherited from the atlas; the town floor is not automatically snow-covered.':''}</p><div class="city-facts"><span>Climate source</span><b>Each world sample</b><span>Ice / biome data</span><b>Inherited, not template colors</b><span>Environment hash</span><b>${g.signature}</b></div><button id="cityShowSetting">Town + surroundings ↗</button> <button id="cityLocateSource">Locate on atlas</button>${layout.townRecipe.compatibilityNote?`<p>${esc(layout.townRecipe.compatibilityNote)}</p>`:''}</section>`;
    }
