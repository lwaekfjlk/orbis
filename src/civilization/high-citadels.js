/** Rare mountain courts founded by existing residents, on their original site.
...
const HighCitadels = (() => {
    const version = 1, surveySpan = 2.16, minimumElevation = 3500, maximumPopulation = 1100;
    function site(w, p) {
    ...
    function historyOptions(sim, overrides = {}) {
        ...
        const highCitadelsVersion = sim.options?.highCitadelsVersion ?? (sim.provinces.some(p => p.highCitadel?.version === version) ? version : 0);
        return {...sim.options, highCitadelsVersion, ...overrides};
    }
    function validate(sim, w) {
        const selected = sim.options?.highCitadelsVersion;
        if (selected !== undefined && selected !== 0 && selected !== version) throw Error('Invalid high citadel founding version.');
        let count = 0;
        for (const p of sim.provinces) {
            const h = p.highCitadel; if (h === undefined) continue;
            if (!h || h.version !== version || !['dragon', 'holy'].includes(h.kind) || ++count > 2 ||
    ...
    function found(sim, w) {
        ...
        for (const c of candidates) {
            ...
            p.highCitadel = {version, kind, originalName, sourceCell: p.i, founded: sim.year, ...c.ground,
    ...
    return {version, surveySpan, minimumElevation, maximumPopulation, site, found, cap, type, historyOptions, validate};
})();
