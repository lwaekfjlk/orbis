...
const E=engine();let w,s,baseline;
...
test.before(async()=>{w=await E.generateWorld(defaults);s=E.createCivilization(w,{realms:18,historySeed:'First-dawn'});baseline=E.createCivilization(w,{realms:18,historySeed:'First-dawn',highCitadelsVersion:0});});
...
test('two real workers prewarm every exact entry while parent buffers remain usable',async()=>{
 ...
 assert.equal(e.queries,2,'only compact high cities need synchronous placement; all ordinary wonders use the verified worker cache');
 ...
 // All 152 ordinary records match baseline 49021ed field for field; worker
 // hydration must preserve the same directory after residential infill.
 ...
 assert.equal(createHash('sha256').update(JSON.stringify(metadata(original))).digest('hex'),'5491ed3269171fbfc1968f8efb8e25dc80c966e7685900c1e60d6c2b52c7bb0d');
 assert.deepEqual(metadata(ordinary),metadata(original),'the current ordinary directory must match the same world with high-city founding disabled');
