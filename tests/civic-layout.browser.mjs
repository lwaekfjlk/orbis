...
const out=resolve(process.env.TELLURIC_CIVIC_OUTPUT||resolve(root,'previews/civic-layout/after'));await mkdir(out,{recursive:true});
...
try {
 ...
 const towns=await page.evaluate(()=>[507,349,168,414,127].map(id=>({id,name:sim.provinces[id].name})));
 for(const {id,name} of towns){
  ...
  assert.equal(state.id,id,'search opened a different named town');
 ...
 const flat=checks.find(x=>x.id===349);
 ...
 await page.setViewportSize({width:430,height:900});await page.waitForFunction('renderer.width===430');await stable();await page.screenshot({path:resolve(out,'mobile.png')});await page.setViewportSize({width:1480,height:980});await page.waitForFunction('renderer.width===1480');
 // Existing underground rooms still need a steeper view down their entrance.
 const underground=398; // The legacy terrain parcel with the recessed Stormbeck entrance.
 await page.evaluate(id=>ContinuousMap.focusTown(id),underground);await stable();
