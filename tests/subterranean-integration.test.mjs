...
test.before(async()=>{
 ...
 for(const name of['Stormbeck','Pineshore']){const p=sim.provinces.find(p=>p.name===name);assert.ok(p,name+' is missing from the default world');const meshes={},renderer={relief:1,zoom:90,world,meshes,upload(name,g,shadow,unlit,alpha){meshes[name]={vertices:new Float32Array(g.data),shadow,unlit,alpha};}},layer=new E.ContinuousCityLayer(renderer);layer.world=world;layer.sim=sim;layer.natural=true;const model=layer.build(p);layer.models.set(p.id,model);
