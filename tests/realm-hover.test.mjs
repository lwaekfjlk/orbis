...
test('Realm hover includes held land and inland water, preserves open wildness and restores the selected realm', () => {
    for (const layer of ['realms', 'diplomacy']) {
        ...
        for (const i of [2, 3, 4, 8]) assert.notDeepEqual(r.palette(i), terrain[i], `${layer}: territory includes its lake and held glacier`);
        for (const i of [6, 7]) assert.deepEqual(r.palette(i), before[i], `${layer}: open wildness is not annexed by hovering a neighboring country`);
...
test('Hover overlays measurement colours and restores each layer exactly on leaving', () => {
    for (const layer of ['faiths', 'peoples', 'wealth', 'magic']) {
        ...
        r.setHoveredRealm(1);
        for (const i of [2, 3, 4, 8]) assert.notDeepEqual(r.palette(i), before[i], `${layer}: all claimed land and water must be visibly identified`);
        for (const i of [0, 1, 5, 6, 7, 9, 10]) assert.deepEqual(r.palette(i), before[i], `${layer}: unrelated measurements and wildness must not change`);
        r.setHoveredRealm(null);
