...
test('The actual OneMap location card trusts neutral status instead of resurrecting a dead province owner',()=>{
    const {w,s}=fixture();s.realms[0].alive=false;
    const before=JSON.stringify(s),view=inspector(w,s);
    assert.equal(E.PoliticalLand.status(w,s,0).label,'No countries yet');
    view.run(0);
    assert.equal(view.cards.length,1);assert.equal(view.cards[0].realm,null);assert.equal(view.cards[0].kicker,'No countries yet');
    ...
    view.run(0);
    assert.equal(view.cards[1].realm,s.realms[1]);assert.equal(view.cards[1].kicker,'Duat');
...
test('Realm profiles and complete territory use identical unit-area fallback while excluding water',()=>{
    const {w,s}=fixture(),before=JSON.stringify(s);
