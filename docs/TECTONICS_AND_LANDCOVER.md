...
加载验收还发现并修复了一个与地下庭院有关的问题：更换世界时，批量卸载旧城会重建即将丢弃的地形。现在先解除旧世界绑定；同世界清理和单城移除仍会补回地面。[调用栈记录](../previews/tectonic-climate/regeneration.json)。合入新版导航按钮前的离线验收约 12.04 秒就绪，首次与重生成均只构建一次地形，173 条地标目录、城市数据、无 Worker 回退和失败恢复全部通过；这是单次记录，见[加载报告](../previews/tectonic-climate/loading/results.json)。
