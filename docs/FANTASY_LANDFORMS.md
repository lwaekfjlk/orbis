...
新世界的 `parameters.landformVersion` 为 `3`。已公开的版本 `1`、`2` 保留原地形；旧存档缺少这个字段时，按 `0` 恢复。四个版本均保留在同一生成器中。最新构造、洋流与颜色规则见[更新说明](TECTONICS_AND_LANDCOVER.md)；本页图片保留早期地貌示例。生成地貌后重新求解水系和气候，再生成居民、城市和国家。近景细化与城市 Worker 读取相同的地形和岩性数据。
