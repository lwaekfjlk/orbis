...
*褶皱山系。数条弯曲山脊共同延伸，各列高峰与鞍部错开，山谷贯穿其间。*
...
同一种子在新地形下的聚落也会重新分布。高空城市仍要求合法的海拔、坡度、气候与承载条件；当前新版默认世界没有满足条件的选址，因此不会强制造城。原地形世界的龙王城和圣城继续保留，已检查其搜索入口、城市模型和存档恢复。

## 验证

改动期间完整回归 465 项通过；完成地貌调整并同步最新主线后，97 项相关集成检查通过。实际离线浏览器确认新版默认的 25 个国家在桌面与 430px 手机视口中均可访问，新旧存档能精确恢复。
