...
try {
    ...
    const report = {sourceCommit: manifest.sourceCommit, bundleSha256: manifest.bundleSha256, grids: [], errors};
    for (const grid of manifest.grids) {
        ...
        const panels = await element.locator('figure').evaluateAll(figures => figures.map(figure => {
            const image = figure.querySelector('img'), rect = figure.getBoundingClientRect();
            return {x: rect.x, y: rect.y, width: rect.width, height: rect.height,
                loaded: image.complete && image.naturalWidth === 1480 && image.naturalHeight === 790};
        }));
        ...
        await element.screenshot({path: join(output, grid.output)});
